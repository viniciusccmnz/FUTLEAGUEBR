...
if(isset($_SESSION['Usuario']) && $id_user2 == $_SESSION['ID']){
?>
<style type="text/css">
#contratosu{
	width:580px;
...
<?php
//pegar dados do dono do contrato
$id1 = $_GET['id_de'];
$id2 = $_GET['id_para'];
$sql = DB::conn()->prepare("SELECT * FROM usuarios WHERE ID = '$id1'");
$sql->execute();
$rs = $sql->fetch();
//dados
$time_de = $rs['Time'];

$movimentacoes = DB::conn()->prepare("SELECT * FROM usuarios WHERE VIP <= '".date('Y-m-d H:i:s')."' AND Time = '$time_de' ORDER BY ID ASC");
...
<span class="nick"><a href="index.php?pr=perfil&usuario=<?php echo utf8_encode(urlencode($nome_user)); ?>" style="color:#000;"><?php echo utf8_encode($nome_user); ?></a></span>
<span class="status">
...
<span class="mudarv"><a href="index.php?pr=alt_vip&id_para=<?php echo $id_user; ?>&id_de=<?php echo  $id1; ?>&id_ant=<?php echo $id2; ?>">Mudar</a></span>
</div>
<?php
}
?>    
