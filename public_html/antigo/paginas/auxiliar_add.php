...
$Cargo = $rs['Cargo'];
$Time = $rs['Time'];
if(isset($_SESSION['Usuario']) && $Cargo == 1 || $Cargo == 2 || $Cargo == 3){
...
$sql = DB::conn()->prepare("UPDATE usuarios SET Cargo = 4 WHERE ID = '$id_para'");
$sql->execute();

	$data = date("d/m");
	$hora = date("H:i");
	$tipo = utf8_decode('Auxiliar');
	$acao = 'Adicionou';
...
echo '<script>alert("OK");location.href="index.php?pr=slot_auxiliar"</script>';	
}else{
echo '<script>alert("Usuario sem VIP");location.href="index.php?pr=slot_auxiliar"</script>';	
}
?>
