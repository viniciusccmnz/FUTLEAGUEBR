...
$Time = $rs['Time'];
$id_de = $rs['ID'];
if(isset($_SESSION['Usuario']) && $trancar == 0 && $Cargo == 1){
?>
<style type="text/css">
#form-contrato input{
	background:#838383;
...
#resultado table tr:hover{
	...
	<tr><td><div style="position:relative;top:0;left:0;width:623;height:57;"><div style="width:623;height:57;background-image:url('img/layout/menu/topH.png');text-aling:center;"><div class="nosel" style="position:absolute;text-align:center;width:623;top:10;color:#515151;font-family:litoh;font-size:1em;">Representantes</div><div style="position:absolute;text-align:center;width:623;top:9;color:#e8f4e1;font-family:litoh;font-size:1em;">Representantes</div></div></div></td></tr>
...
<center>
<div id="id_vips" style="font-family: Helvetica, sans-serif, Arial; font-size:16px; margin-bottom:5px;">Adicionar Representantes</div>
...
$query = DB::conn()->prepare("SELECT ID FROM usuarios ORDER BY Gols_Total DESC");
$query->execute();
while($rs = $query->fetchObject()){
	...
	while($rs = $selecionar_contratos->fetch()){
		$cargos = $rs['Cargo'];
		$representantes = $rs['Representante'];
...
?>
  ...
  <tr>
    ...
    <div id="user" style="line-height:25px; float:left; margin-left:5px;"><?php if ($rs["VIP"] > 0 && $rs['VIP'] >= date('Y-m-d H:i:s')) { ?><span class='cVip'><?php echo $rs['Usuario'];?></span><?php } else { ?><span class='cVipx' style="color:#333;"><?php echo $rs['Usuario'];?></span><?php } ?>
   ...
   if($representantes == 1){
   ?>
    <a href="index.php?pr=remover_representantes&id_para=<?php echo $rs["ID"] ?>&id_de=<?php echo $id_de; ?>" style="color:#FFF; font-size:12px;">Remover</a>
    <?php }else{ ?>
     <a href="index.php?pr=add_representantes&id_para=<?php echo $rs["ID"] ?>&id_de=<?php echo $id_de; ?>" style="color:#FFF; font-size:12px;">Adicionar</a>
     <?php } ?>
    </span>
    </center>
    </td>
