...
<table width="274" border="1" cellpadding="0" cellspacing="0">
  <tr style="background:#666;">
    <td width="177" height="24">Usuário</td>   
    <td width="91"></td>
...
while($rs = $query->fetchObject()){
	...
	?>
	<?php
		$select = DB::conn()->prepare("SELECT * FROM usuarios WHERE Cargo = 1 AND Time = '$Time'");
		$select->execute();
		$rs = $select->fetch();
...
?>
    ...
    <div id="user" style="line-height:25px; float:left; margin-left:5px;">
     <?php if ($rs['VIP'] >= date('Y-m-d H:i:s')) { ?><span class='cVip'><?php echo utf8_encode($rs['Usuario']);?></span><?php } else { ?><span class='cVipx' style="color:#333;"><?php echo utf8_encode($rs['Usuario']);?></span><?php } ?><img src='img/c1.png' border=0>
     </div>
    </td>
    ...
    <a href="index.php?pr=sair_presidencia&id=<?php echo $rs['ID']; ?>" style="color:#FFF; font-size:12px;">Remover</a>
