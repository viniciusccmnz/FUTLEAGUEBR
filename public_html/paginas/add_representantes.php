...
if($Rodada >= 2){
echo '<script>alert("O prazo de adicionar representantes se esgotou !");location.href="index.php?pr=index"</script>';	
}else{
...
$sql = DB::conn()->prepare("UPDATE usuarios SET Representante = 1 WHERE ID = '$id_para'");
$sql->execute();

	$data = date("d/m");
	$hora = date("H:i");
	$tipo = utf8_decode('Representante');
	$acao = 'Adicionou';
...
echo '<script>alert("OK");location.href="index.php?pr=representantes"</script>';	
}else{
echo '<script>alert("Usuario sem VIP");location.href="index.php?pr=representantes"</script>';	
}
?>
